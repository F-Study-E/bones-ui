...
- `@bones-ui/react`에 첫 상태 컴포넌트인 **Switch** 추가 (Switch.Root + Switch.Thumb)
- 그 과정에서 토대 정리:
  - `@bones-ui/hooks` 패키지 부트스트랩 + `useControllableState` 신설
...
**오늘 목표**
- Switch 컴포넌트 + 스토리 + 테스트
- @bones-ui/hooks 부트스트랩 + useControllableState
- 도트 표기 export 컨벤션 채택
- 4개 커밋으로 분리 (chore / refactor / feat-hooks / feat-switch)

**한 것**
- `@bones-ui/hooks` 패키지 신설 — `package.json`, `tsconfig`, `tsup`, `vitest` 설정. `useCallbackRef`, `useControllableState` 구현 + 단위 테스트 9개 (모두 통과)
...
- changeset(@bones-ui/react minor + @bones-ui/hooks minor) 추가
...
- 단위 테스트는 통과했지만 `tsup`의 dts 산출 단계가 `tsc`를 실제로 돌려 실패 → `dist/index.d.ts` 생성 안 됨 → docs(IDE) 측에서 `@bones-ui/react` 타입을 못 찾는 부작용까지
...
| `useControllableState`를 `@bones-ui/hooks`에 신설 | `switch` 폴더 안에 로컬 훅 | Dialog/Popover/Checkbox에서 동일하게 재사용. 공용 훅 패키지의 첫 입주자 |
...
| `composeEventHandlers`를 `switch.utils.ts`에 위치 | 처음부터 `@bones-ui/utils` | YAGNI. 두 번째 컴포넌트(Dialog 등)에서 import하기 시작하면 그때 승격 |
...
- [ ] `composeEventHandlers` 두 번째 컴포넌트가 import 시작하면 `@bones-ui/utils` 또는 새 위치로 승격
- [ ] 사전 존재 changeset `primitive-slot-compose-refs.md`가 제거된 `@bones-ui/primitive` 패키지를 참조 — `version-packages` 단계에서 stale 경고 가능. 별건으로 정리
